...
- 题库缓存：可选的按题目语义、站点、模型和思考强度缓存答案，不执行全量题库扫描；默认关闭
- 课程选择：支持刷新课程列表后按块选择课程
- 批量操作：支持批量启动、批量停止、批量删除
- 进度控制：阅读停留时长、任意正倍速、任意正并发数；直播任务固定 1 倍速
- 完成确认：任务执行后通过课程知识点接口重新确认完成状态；已过期、平台明确不可提交的作业按跳过处理，不阻断课程，其它失败仍会报错；挑战模式支持有限次重试
...
1. 在 `配置管理` 的 Responses AI 区域填写站点 JSON、当前站点、模型和思考强度；需要复用答案时再勾选“启用语义缓存（默认关闭）”
