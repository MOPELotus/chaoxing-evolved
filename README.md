...
- 自定义题库：内置支持 `荷花题库`，仅需填写接口地址即可接入
- 任务中心：支持检索独立作业、考试与课程活动，并提供循环监控入口
...
- `命令行 Pro`：用于通过独立控制台窗口分别管理配置、全局设置、运行监控与实时日志
- `任务中心`：用于查看待处理作业、考试与课程活动，可一键打开任务页或启用循环监控
...
3. 如需接入 `荷花题库`，在全局设置或单个档案中填写对应的 `url`
4. 通过课程块选择课程，通过题库块选择协同题库
5. 在 `概览` 页查看运行日志，并按需启动或停止任务
